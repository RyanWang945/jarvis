...
def _build_node_result(context, agent_result: dict[str, Any]) -> NodeResult:
    ...
    evidence_artifacts = _evidence_artifacts_from_tool_calls(context, tool_calls)
    if evidence_artifacts and not ok and agent_result.get("error_code") == _CODE_MAX_TURNS_NO_FINAL_OUTPUT:
        ok = True
        node_status = "completed"
        summary = (
            summary
            if summary and summary != _mark_max_turns_no_final_output({}).get("summary")
            else "Evidence claims artifact generated."
        )
    ...
    node_artifacts = _dedupe_node_artifacts([*_artifacts_from_data(data), *evidence_artifacts])
    return NodeResult(
...
def _evidence_artifacts_from_tool_calls(context, tool_calls: list[dict[str, Any]]) -> list[NodeArtifact]:
    result: list[NodeArtifact] = []
    session_root = context.runtime_context.workspace.session_root
    for call in tool_calls:
        tool_name = str(call.get("tool_name") or "").strip()
        if tool_name not in {"Write", "write_file", "mcp__jarvis__write_file"}:
            continue
        args = call.get("args")
        if not isinstance(args, dict):
            continue
        path_text = str(args.get("file_path") or args.get("path") or "").strip()
        if not path_text:
            continue
        path = Path(path_text)
        filename = path.name.lower()
        if filename not in _EVIDENCE_ARTIFACT_FILENAMES:
            continue
        session_relative_path = _session_relative_path(path, session_root)
        result.append(
            NodeArtifact(
                ref=path.name,
                kind="file",
                name=f"{context.node.id}/{path.name}",
                description=f"Evidence claims generated by node {context.node.id}.",
                path=str(path),
...
def _session_relative_path(path: Path, session_root: Path | None) -> str | None:
    if session_root is None:
        return None
    try:
        return path.expanduser().resolve(strict=False).relative_to(session_root.resolve(strict=False)).as_posix()
    except (OSError, ValueError):
        return None


def _dedupe_node_artifacts(artifacts: list[NodeArtifact]) -> list[NodeArtifact]:
    result: list[NodeArtifact] = []
    seen: set[tuple[str, str | None]] = set()
    for artifact in artifacts:
        key = (artifact.ref, artifact.session_relative_path or artifact.path)
        if key in seen:
            continue
        seen.add(key)
