...
def test_multiple_tool_calls_in_one_assistant_message_share_step_index(monkeypatch) -> None:
    ...
    chat = ScriptedChat([
        tool_response(
            tool_call("shell_inspect", {"command": "pwd"}, call_id="call_pwd"),
            tool_call("shell_inspect", {"command": "git status"}, call_id="call_git_status"),
        ),
    ...
    def _fake_execute_tool(tool, tool_args, *, timeout_seconds=30):
        return ToolExecutionResult(
            ok=True,
            exit_code=0,
            stdout=f"out:{tool.name}:{tool_args['command']}",
            summary="ok",
        )

    monkeypatch.setattr(react_graph, "execute_tool", _fake_execute_tool)
    created = create_dm_turn(client, "Inspect the workspace and git status.")
    ...
    assert [record.tool_name for record in tool_calls] == ["shell_inspect", "shell_inspect"]
    assert [record.provider_tool_call_id for record in tool_calls] == ["call_pwd", "call_git_status"]
...
def test_tool_exception_is_audited_and_returned_to_model(monkeypatch) -> None:
    ...
    store = get_conversation_store()
    chat = ScriptedChat([
        tool_response(tool_call("shell_inspect", {"command": "pwd"}, call_id="call_explodes")),
