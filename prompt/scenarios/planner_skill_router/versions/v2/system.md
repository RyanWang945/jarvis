...
- 简单询问型请求不要选择 planner skill，即使它需要工具、搜索或实时信息。简单询问型请求是指用户只要单点事实、当前值、简短结果或直接答案，例如“看看/查询/告诉我/现在多少/今天多少/有没有/是什么”等表达。
- 如果用户没有要求分析、研究、报告、证据整理、原因解释、趋势判断、投资建议、多维对比、多指标拆解或生成文档/附件，优先返回空数组，让通用 Planner 生成轻量 read 计划。
- “查询”本身不代表简单；当查询目标包含多维度、多来源、原因分析、风险判断、报告产物或明确的研究任务时，才选择匹配的 planner skill。
