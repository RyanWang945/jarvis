...
已执行迁移：新增 `app/task_runtime/runtime_context.py`，提供 `TurnRuntimeContext`、`TemporalRuntimeContext`、`RepoRuntimeContext`、`BranchRuntimeContext`、`NodeWorkspaceRuntimeContext`、`WorkspaceRuntimeContext` 和聚合 `RuntimeContext`。`agent_runtime.py` 已在 turn 入口构建 `RuntimeContext`，session workspace 合并通过 `RuntimeContext.with_hints()` 完成；`NodeExecutionContext` 主字段已改为 `legacy_hints`，并在初始化时从 legacy hints 构建 `runtime_context`，旧 `runtime_hints` property 已删除。`PlanningRouter`、`TurnPlanner`、`FastIntentNode`、`NodeExecutor` 和 `ResultAggregator` 已改为只接收 `RuntimeContext`，`TaskAgentRuntime` 主链路已不再向内部服务传 `runtime_hints` 参数；评测脚本与 fixture 已改为 `runtime_context` 字段，loader 仅保留读取旧 `runtime_hints` 的兼容分支。`PlanInput` 已拆出不进入 prompt JSON 的 `typed_runtime_context`，并将 planner prompt JSON 的顶层字段从 `runtime_hints` 改为 `runtime_context`；对应新增 `heavy_plan:v6` 并切为默认版本。`ResultAggregator` prompt JSON 也已从 `runtime_hints` 改为 `runtime_context`；对应新增 `result_aggregator:v5` 并切为默认版本。`llm_node_execute` / `react_node_execute` prompt JSON 也已从 `runtime_hints` 改为 `runtime_context`；对应新增 `llm_node_execute:v2`、`react_node_execute:v3` 并切为默认版本。`NodeExecutor` 的 progress turn/conversation id 读取和 session/node workspace hints 合并已改为通过 `RuntimeContext`；`PlanningRouter` 的 progress id 与 active repo 路由判断、`planner.py` 的 fallback repo 判断也已改为通过 `RuntimeContext`。`fast_intent.py` 和 `node_execute_runtime.py` 中的 temporal、repo、provider run dir、branch、manifest path、session workspace/node workspace 读取已改为通过 typed accessor；coder git context LLM usage 已从 legacy hints 桥接迁出到 `NodeExecutionContext.usage_records`，再进入 provider request metadata 的 `usage_records`。`fast_intent.py` 的 prompt payload 边界已改为显式从 `RuntimeContext` 导出 legacy hints；`SessionWorkspaceRef` / `NodeWorkspaceRef` 已只保留 `to_legacy_hints()` 作为旧 payload 输出 API，旧 `runtime_hints()` 别名已删除；`session_workspace.py` 中 repo worktree 准备路径已改为通过 typed accessor 读取 node repo dir、session id、branch 和 session root，node input snapshot 的 raw payload 字段也已从 `runtime_hints` 改为 `runtime_context`；内部参数命名使用 `legacy_hints` 表达旧 dict 格式边界。
